import { BlobInventoryRepository } from "@/server/repositories/blob/BlobInventoryRepository";
import { BlobTransactionRepository } from "@/server/repositories/blob/BlobTransactionRepository";
import type { InventoryItem, InventoryLedgerEntry } from "@/types";

export class InventoryService {
  private inventoryRepo = new BlobInventoryRepository();
  private transactionRepo = new BlobTransactionRepository();
   ...
   * items (wheat/rice) always reflect actual synced distribution rather than
   * a stored snapshot, so the ledger can't drift from real distribution data.
   */
