...
export function extractTotalsFromHtml(html: string): {
  totalWheat: number;
  totalRice: number;
  totalSaree: number;
  totalAmount: number;
  ...
  const $ = cheerio.load(html);
  const footerRow = $("#Report tfoot tr th");

  return {
    totalWheat: parseFloat($(footerRow[1]).text().trim()) || 0,
    totalRice: parseFloat($(footerRow[2]).text().trim()) || 0,
    totalSaree: parseFloat($(footerRow[3]).text().trim()) || 0,
    totalAmount: parseFloat($(footerRow[4]).text().trim()) || 0,
  };
}
